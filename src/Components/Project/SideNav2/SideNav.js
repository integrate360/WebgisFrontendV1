...
const SideNav = ({ adjustHeight, width }) => {
  const { map, source, setMap, setSource } = useMap();
  const { editOptions, setEditOptions, setLabel } = useEditOptions();
  const { annotations, setAnnotations, updateAnnotation } =
  ...
  return (
    <div
      ...
      style={{
        height: adjustHeight ? "calc(97vh - 148px)" : "calc(97vh - 48px)",
        width: `${width}%`,
      }}
    >
      <div id="edit-options">
            ...
            {!["kml", "kmz", "shp", "shz", "dxf"].includes(
              ...
              <>
                ...
                <h4 className="ll-text">
                  Latitute <b style={{ color: "white" }}>{lat}</b>
                </h4>
                <h4 className="ll-text">
                  Longitude <b style={{ color: "white" }}>{lon}</b>
                </h4>
                <hr className="light-hr" />
              </>
            )}
