...
const AnnotationItemCompare = ({ data, index, source, map }) => {
  ...
  function getLayerById(id) {
    const layers = maps?.getLayers().getArray();
  ...
  const checkboxHandler = () => {
    ...
    handleCheckboxChange(data.data.id, !isChecked, sources, data?.featureId);
  };
  ...
  const clickHandler = (id) => {
    ...
    // righteToggle.innerHTML = `<i className="fas fa-chevron-left"></i>`;
    ...
    if (data.type === "pin") {
      ...
      maps?.getView().setCenter(layer?.getSource()?.getExtent());
