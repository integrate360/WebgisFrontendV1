...
export function fetchFormatArea(polygon, unit) {
  let output;
  if (!polygon) return (output = "");
  switch (unit) {
    case "meters":
      if (polygon) output = polygon?.toFixed(6) + " m²";
      break;
    case "kilometers":
      if (polygon) output = (polygon / 1e6)?.toFixed(6) + " km²";
      break;
    case "miles":
      if (polygon) output = (polygon / 2.58999e6)?.toFixed(6) + " mi²";
      break;
    case "feet":
      if (polygon) output = (polygon * 10.7639)?.toFixed(6) + " ft²";
      break;
    case "acres":
      if (polygon) output = (polygon / 4046.86)?.toFixed(6) + " acres";
      break;
    case "hectares":
      if (polygon) output = (polygon / 10000)?.toFixed(6) + " hectares";
      break;
    default:
      output = "Unit not supported";
  }
...
//       updateFeatureStyle
//     );
//     editOptions.appendChild(createLabeledElement("Fill:", fillColorInput));
//   }
